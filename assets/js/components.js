...
function make_control_bar(page) {
    let result = '';
    if (config[page]["searchbar"]){
        result += `<input type="text" class="searchbar" placeholder="Поиск">`;
    }
    if (config[page]["delete_button"]){
        result += `<button id="delete_button">
    ...
    if (config[page]["show_hide_button"]){
        result += `<button>Показать закрытые</button>`;
    }
    if (config[page]["create_button"]){
        result += `<button id="create_button">
