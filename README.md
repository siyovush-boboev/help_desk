...
Frontend всегда обращается к бекенду по относительным путям (`/api`, `/uploads`), без указания хоста и порта — в проде это работает благодаря reverse proxy (IIS), который проксирует эти пути на Go-бекенд.

Для локальной разработки Vite dev-сервер проксирует `/api` и `/uploads` на бекенд, адрес которого задан в `vite.config.js` (по умолчанию — общий тестовый сервер). Если вы запускаете бекенд локально или на другом адресе, создайте файл `.env.local` в корне проекта:

```
VITE_DEV_BACKEND_TARGET=http://127.0.0.1:8080
```
